 ...
 */
public class SpotifyContentProvider extends android.content.ContentProvider
{
    // constants
    public final String LOG_TAG = SpotifyContentProvider.class.getSimpleName();
...
//        matcher.addURI(authority, WeatherContract.PATH_LOCATION + "/#", LOCATION_ID);
    ...
    private java.util.ArrayList<java.util.ArrayList> parseTrackJSON (String json)
    {
        ...
        try {
          if (json != null) {
            ...
            if (trackArray != null) {
                ...
                java.util.HashMap<String, String> savedImageUrls = new java.util.HashMap<String, String>();
                for (int j=0, numTracks=trackArray.length(); j < numTracks; j++) {
                    ...
                    String smallestImageUrl = null, largestImageUrl = null;
                    if (album != null) {
                        org.json.JSONArray albumImages = album.getJSONArray("images");
                        if (albumImages.length() > 0) {
                            int smallestWidth = 0, largestWidth = 0;
                            for (int k=0, numImages=albumImages.length(); k<numImages; k++) {
                                ...
                                if ((smallestWidth == 0) || (thisWidth < smallestWidth)) {
                                    ...
                                    smallestImageUrl = image.getString("url");
                                }
                                if ((largestWidth == 0) || (thisWidth > largestWidth)) {
                                    largestWidth = thisWidth;
                                    largestImageUrl = image.getString("url");
                                }
                            }
                        }
                    }
                    row.add(smallestImageUrl);

                    savedImageUrls.put(trackTitle, largestImageUrl);
     ...
     *      that _id value.
     * @param requestedColumnNames The list of columns to put into the cursor. If
     ...
     */
    @Override
    public android.database.Cursor query(android.net.Uri uri, String[] requestedColumnNames, String selection, String[] selectionArgs, String sortOrder) {
...
//                retCursor = cursor;
                    String jsonResponse = SpotifyWebApiClient.searchTracks(selectionArgs[0]);
                    java.util.ArrayList<java.util.ArrayList> rows = parseTrackJSON(jsonResponse);
                    String[] columnMap = new String[]{"id", "title", "artistNames", "previewUrl", "iconUrl"};
                    retCursor = new com.android.common.ArrayListCursor(columnMap, rows);
                }
                break;
